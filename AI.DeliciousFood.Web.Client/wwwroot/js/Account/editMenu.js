var currentValue = 2;
var uploadedFiles = [];     // 新上传[本次上传但未入库]
var deletedInitialFiles = [];   // 记录被删的“初始图片”文件名
var html;

// 获取图片预览容器上的 data 值
var $previewContainer = $('#image-preview');
var initialPreview = $previewContainer.data('preview')
    ? $previewContainer.data('preview').split(',')
    : [];
var initialPreviewConfig = $previewContainer.data('preview-config') || [];

// 初始图片文件名，你可以在页面渲染时写入
var initialFileNames = initialPreviewConfig.map(function (item) {
    return item.key;
});


// algo核心：实时生成**现有“未被删除”的初始图片文件名”
function getCurrentInitialFiles() {
    // 被标记删除的都不要
    return initialFileNames.filter(function (name) {
        // 只保留没被删除的
        return deletedInitialFiles.indexOf(name) === -1;
    });
}
...
$("#input-ke-2").fileinput({
    ...
    initialPreview: initialPreview,                  // 👈 这里放图片地址数组
    initialPreviewAsData: true,                       // 👈 告诉插件预览内容是图片地址，而不是原始HTML
    initialPreviewConfig: initialPreviewConfig,        // 👈 这里放预览配置
...
// 选择新文件时的去重判断
$('#input-ke-2').on('filebatchselected', function (event) {
    var input = document.getElementById('input-ke-2');
    var dt = new DataTransfer();

    // 获取所有“未被删除的初始图片文件名”
    var initialNameSet = new Set(getCurrentInitialFiles());

    var duplicate = [];
    var hasInitialDuplicate = false;

    for (let i = 0; i < input.files.length; i++) {
        let fname = input.files[i].name;
        // ⭐⭐ 重点：只判断和“初始图片”重名的情况 ⭐⭐
        if (initialNameSet.has(fname)) {
            hasInitialDuplicate = true;
            if (initialNameSet.has(fname)) {
                duplicate.push(fname);
            } else {
    ...
    // ⚠️**只有有“初始图片名”重复才处理（否则什么都不做，留给插件处理）**
    if (hasInitialDuplicate && duplicate.length > 0) {
        input.files = dt.files;

        // === 只移除新添加重复文件的缩略图，不动初始的 ===
        $('.file-preview-frame').not('.file-preview-initial').filter(function () {
            // 兼容blob文件的情况，data-filename 可能不存在需容错
            var filename = $(this).data('filename') ||
                $(this).find('.file-footer-caption').attr('title') ||
                $(this).find('.file-caption-info').text().trim();
            return duplicate.indexOf(filename) !== -1;
        }).each(function () {
            // 彻底移除整个预览节点
            $(this).remove();
        });
        ...
        // ---- ✅ 推荐的错误消息插入方式如下： ----
        var messageHtml = duplicate.map(name =>
            `<li>文件 "<b>${name}</b>" 已经被选中,忽略相同的文件。</li>`
        ).join('');

        // 保证结构和 fileinput 一致
        var html = `
            <button type="button" class="btn-close kv-error-close float-end close-error-message" aria-label="Close"></button>
            <ul>${messageHtml}</ul>
        `;

        setTimeout(function () {
            var $msgBox = $('.file-error-message');
            $msgBox.css("position", "relative").html(html).show();
            $msgBox.off('click', '.close-error-message').on('click', '.close-error-message', function () {
                $msgBox.hide();
            });
        }, 50);
    }
    // 否则，不做任何处理（让bootstrap-fileinput自身控制重名）
});
...
// 删除新上传图片（移除 uploadedFiles 中对应项）
$('#input-ke-2').on('filesuccessremove', function (event, previewId) {
    var index = uploadedFiles.findIndex(f => f.previewId === previewId);
    if (index !== -1) {
        uploadedFiles.splice(index, 1);
        console.log('删除上传图片: ', previewId);
    }
});

// 删除初始图片（记录 key 到 deletedInitialFiles）
$("#input-ke-2").on('filepredelete', function (event, key, jqXHR, data) {
    deletedInitialFiles.push(key);
    console.log('已标记待删除的初始图片:', deletedInitialFiles);
    // 不要 return abort: true，否则不会删DOM
});
...
function handleMenuSubmit(event, isDraft) {
    ...
    // 当前“未被删除的初始图片”集合（文件名），用于校验
    const retainedInitialFileNames = getCurrentInitialFiles();
    // 对应结构补全，用于校验
    const retainedFiles = initialPreviewConfig
        .filter(item => retainedInitialFileNames.includes(item.key))
        .map(item => ({
            fileName: item.key,
            isInitial: true,
            previewUrl: item.url
        }));
    // 合并成现在页面存在的全部图片，仅用于校验
    const allFilesForValidate = [
        ...retainedFiles,
        ...uploadedFiles
    ];
    ...
    var menuData = {
        ...
        Taste: $('#taste').val() || null,
        CookingCraft: $('#cookingCraft').val() || null,
        RecipeCategories: $('#recipeCategories').val() || null,
    ...
    if (isDraft) {
        ...
        var errorMessage = validateMenuData(menuData, allFilesForValidate.length);
    }
...
function validateMenuData(menu, currentPicCount) {
    const checks = [
        { condition: !menu.RecipeName, message: "请输入菜谱名称" },
        { condition: !currentPicCount || currentPicCount === 0, message: "请上传图片" },
